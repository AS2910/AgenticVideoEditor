...
```sh
cd backend && .venv/bin/python -m pytest      # 354 tests
cd frontend && npm test                        # 108 tests, 17 files
```
...
Deferred by design: the speaker's own cloned voice (Phase 4b) and real lip-sync (Phase 5) — both in the roadmap's backlog, blocked on vendor access — plus voice-identity and lip-sync scoring (Phase 6b, after those), sign-in (Phase 9c, designed), and multiple voices — speaker labels on the transcript, and a voice per speaker. See `docs/superpowers/plans/2026-09-22-real-pipeline-roadmap.md` for the phased route through them. Product-level, v1 is dialogue changes only — visual detail swap (Phase 2), pacing/filler edits, multi-speaker crosstalk, and non-English are all out of scope.
